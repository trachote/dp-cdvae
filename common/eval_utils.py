...
from dpcdvae.predict_property import CrystGNN_Supervise
...
def get_model_path(eval_model_name):
    import dpcdvae
    model_path = (
        Path(dpcdvae.__file__).parent / 'prop_models' / eval_model_name)
    return model_path
...
def load_model(MODEL, model_path, load_data=False, testing=True, prop_model=False):
    cfg = OmegaConf.create(OmegaConf.to_container(OmegaConf.load(str(model_path/'hparams.yaml')), resolve=True))
    hparams = cfg.model
