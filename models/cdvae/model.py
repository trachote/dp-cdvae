...
from .gnn import DimeNetPlusPlusWrap
...
class CrystGNN_Supervise(MODEL):
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.encoder = DimeNetPlusPlusWrap(**{k:v for k, v in self.hparams.encoder.items() if k!= '_target_'})
        # num_targets = 1
...
class CDVAE(MODEL):
    def __init__(self, cfg) -> None:
        ...
        self.encoder = DimeNetPlusPlusWrap(**params)

        self.decoder = GemNetTDecoder(**{k:v for k, v in self.hparams.decoder.items() if k!= '_target_'})
        ...
        self.prop = list(self.hparams.data.prop)
        self.prop_classes = list(self.hparams.data.prop_classes)
        self.prop_num_classes = list(self.hparams.data.prop_num_classes)

        # for property prediction.
        if self.hparams.predict_property:
            self.fc_property = torch.nn.ModuleList()
            for i in self.hparams.data.prop:
                self.fc_property.append(build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim, self.hparams.fc_num_layers, 1))

            # self.fc_property = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
            #                              self.hparams.fc_num_layers, 1)

            self.len_prop = len(self.hparams.data.prop)

        # for property class prediction.
        if self.hparams.predict_property_class:
            self.fc_property_class = torch.nn.ModuleList()
            for i in self.hparams.data.prop_num_classes:
                self.fc_property_class.append(build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim, self.hparams.fc_num_layers, i))

            self.len_prop_classes = len(self.hparams.data.prop_num_classes)
    ...
    def forward(self, batch, teacher_forcing, training, model_classifier=None):
        ...
        if model_classifier is not None:
            return model_classifier(z, noisy_frac_coords, rand_atom_types, batch.num_atoms, pred_lengths, pred_angles)
