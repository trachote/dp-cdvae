...
class GemNetT(torch.nn.Module):
    ...
    def __init__(
        ...
        ## Additions
        #self.mlp_force_time = nn.Sequential(nn.Linear(129, 129), nn.ReLU(), nn.Linear(129, 1))
        
        if self.regress_logvars:
            self.mlp_mu = nn.Sequential(nn.Linear(1, 128),
                                            nn.ReLU(), 
                                            nn.Linear(128, 3)
                                           )
            self.mlp_logvar = nn.Sequential(nn.Linear(1, 128),
                                            nn.ReLU(), 
                                            nn.Linear(128, 3)
                                           )
            for i in [0, 2]:
                nn.init.zeros_(self.mlp_mu[i].weight)
                nn.init.zeros_(self.mlp_mu[i].bias)
                nn.init.zeros_(self.mlp_logvar[i].weight)
                nn.init.zeros_(self.mlp_logvar[i].bias)
    ...
    def forward(self, z, time_emb, pos, atom_types, num_atoms, 
                lengths, angles, edge_index, to_jimages, num_bonds, 
                extra_feats=None):
        """
        args:
        ...
        if self.frac_to_cart:
            pos = frac_to_cart_coords(pos, lengths, angles, num_atoms)
        ...
        # Edge batch
#         batch = torch.arange(num_atoms.shape[0], device=num_atoms.device)
#         batch = batch.repeat_interleave(num_atoms, dim=0)
#         edge_batch = batch[idx_s]
        ...
        if (z is not None) or (time_emb is not None):
            ...
            if time_emb is not None:
                time_emb = time_emb.repeat_interleave(num_atoms, dim=0)
                h_per_atom.append(time_emb)
            if extra_feats is not None:
                h_per_atom.append(extra_feats)
        ...
        E_t, A_t, F_st = self.out_blocks[0](h, m, rbf_out, idx_t)
        ...
        for i in range(self.num_blocks):
            ...
            E, A, F = self.out_blocks[i + 1](h, m, rbf_out, idx_t)
            # (nAtoms, num_targets), (nEdges, num_targets)
            F_st += F
            E_t += E
            A_t += A
        ...
        if self.regress_forces:
            ...
            if time_emb is not None:
                pass#F_st = torch.cat([F_st, time_emb[idx_s]], dim=-1)
                #F_st = self.mlp_force_time(F_st)
                
            # map forces in edge directions
            F_st_vec = F_st[:, :, None] * V_st[:, None, :]
            
            if self.regress_logvars:
                mu = self.mlp_mu(F_st)[:, None, :] #* V_st[:, None, :]
                logvar = self.mlp_logvar(F_st)[:, None, :] #* V_st[:, None, :]
                F_st_vec = torch.cat([F_st_vec, mu, logvar], dim=-1)
#                 logvar = self.mlp_logvar(torch.cat([F_st, time_emb[idx_s]], dim=-1))[:, None, :]
#                 F_st_vec = torch.cat([F_st_vec, logvar], dim=-1)
            ...
            return E_t, A_t, F_t  # (nMolecules, num_targets), (nAtoms, 3)
        else:
            return E_t
