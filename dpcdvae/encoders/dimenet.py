...
class DimeNaEncoder(nn.Module):
    def __init__(self, hparams):
        super().__init__()
        latent_dim = hparams.latent_dim
        dime_params = {k: v for k, v in hparams.encoder1.items() if k != "_target_"}
        na_params = {k: v for k, v in hparams.encoder2.items() if k != "_target_"}
        
        self.dime = DimeNetppEncoder(**dime_params)
        self.emb_na = nn.Embedding(na_params['max_atoms']+1, na_params['latent_dim'])
        
        fc_dim = dime_params['num_targets'] + na_params['latent_dim']
        self.fc = nn.Sequential(nn.Linear(fc_dim, 2*fc_dim),
                                nn.ReLU(),
                                nn.Linear(2*fc_dim, latent_dim))
       
    def forward(self, data):
        z1 = self.dime(data)
        z2 = self.emb_na(data.num_atoms)
...
class DimeGINNaEncoder(nn.Module):
    def __init__(self, hparams):
        super().__init__()
        latent_dim = hparams.latent_dim
        dime_params = {k: v for k, v in hparams.encoder1.items() if k != "_target_"}
        gine_params = {k: v for k, v in hparams.encoder2.items() if k != "_target_"}
        na_params = {k: v for k, v in hparams.encoder3.items() if k != "_target_"}
        
        self.max_atoms = na_params['max_atoms']
        self.dime = DimeNetppEncoder(**dime_params)
        self.gine = GINEncoder(**gine_params)
        self.emb_na = nn.Embedding(na_params['max_atoms']+1, na_params['latent_dim'])
#         self.emb_na = nn.Sequential(nn.Linear(self.max_atoms+1, na_params['latent_dim']),
#                                     nn.ReLU(),
#                                     nn.Linear(na_params['latent_dim'], na_params['latent_dim']))
        
        fc_dim = dime_params['num_targets'] + gine_params['latent_dim'] + na_params['latent_dim']
        self.fc = nn.Sequential(nn.Linear(fc_dim, fc_dim),
                                nn.ReLU(),
    ...
    def forward(self, data):
        ...
        z2 = self.gine(data)
        z3 = data.num_atoms
        #z3 = one_hot(z3, self.max_atoms+1).float()
        #z3 = z3 + torch.randn_like(z3) * 0.01
        z3 = self.emb_na(z3)
