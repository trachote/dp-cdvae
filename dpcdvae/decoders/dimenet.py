...
class BesselBasisLayer(nn.Module):
    ...
    def reset_parameters(self):
        ...
        self.freq = nn.Parameter(torch.arange(1, self.freq.numel() + 1, dtype=self.freq.dtype).mul_(PI))
...
class DimeNetpp(nn.Module):
    ...
    def __init__(
        ...
        self.fc_atom = nn.Sequential(nn.Linear(2*hidden_channels, hidden_channels),
                                     nn.ReLU(),
    ...
    def forward(self, z, data):
        ...
        if batch is None:
            ...
            energy = scatter(P, batch, dim=0, dim_size=data.num_atoms.sum(), reduce='add')
